...
class UsersController extends AppController {
	...
	function autocomplete()
	{
    	...
    	$users = $this->User->findAll( "User.name LIKE '%{$this->data['User']['name']}%'", array('User.name','User.email'), 'User.name ASC', 20);
	...
	function upload() {
		...
		if (empty($this->data)) {
			...
			if (!$result)
			{
				$this->data['User']['avatar_url'] = '/v2/img/uploads/avatars/' . $this->Upload->result;
