<div class="comments index">
	...
	<?php
		...
		<tr>
			<td>
				<?php echo substr($comment[$commentsModel]['created'], 2, 9);?>
				<br/>
				<?php echo $html->link($comment['User']['username'], array('controller'=>'users','action'=>'view',  $comment['User']['id'])); ?>				
			</td>		
			<td style="width:auto;vertical-align: top;" rowspan="2">
				<?php echo $comment[$commentsModel]['body'];?>
			</td>
	...
	<br/>
	<div class="paging">
	<?php echo $paginator->prev('<< '.__('previous', true), array('model' => $commentsModel), null, array('class'=>'disabled'));?>
 | 	<?php echo $paginator->numbers(array('model' => $commentsModel));?>
	<?php echo $paginator->next(__('next', true).' >>', array('model' => $commentsModel), null, array('class'=>'disabled'));?>
	</div>
	<br/>	
	<?php if($othAuth->sessionValid()): ?>
	<div class="infocomments form">
	
		<?php echo $form->create((isset($controller) ? $controller : null), array('action' => (isset($action) ? $action : 'addcomment')));?>			 	
	...
	<?php else: ?>
	<div class="infocomments form">		
		<?php __("Logon if you want to leave a message"); ?>
	</div>
	<?php endif; ?>	
