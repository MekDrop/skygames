...
class TeamsController extends AppController {
	...
	function autocomplete($game_id, $type = null)
	{
		...
		$conditions = array ("OR" => array("name LIKE '%" . $this->data['Team']['name'] . "%'", "tag LIKE '%" . $this->data['Team']['name'] . "%'"));
	...
	function invite($id = null)
	{
		...
		if (!empty($this->data))
		{
			...
			$users = $this->Team->User->findAll(array('name'=>$this->data['User']['name']));
			...
			if (count($users) == 1)
			{
				...
				clearCache('cache_element_plugin_'.$users[0]['User']['id'].'loginbox' , 'views', '.');
	...
	function add($id = null) {
		...
		if (!empty($this->data)) {
			...
			if ($result = $this->Team->save($this->data)) {
				...
				clearCache('element_cache_plugin_'.$this->othAuth->user('id').'loginbox' , 'views', '.');
		...
		$events = $this->Team->Pool->Event->find('list');
	...
	function delete($id = null) {
		...
		else
		{
			...
			{
				if ($this->Team->del($id)) {
					clearCache('element_cache_plugin_'.$id.'loginbox' , 'views', '.');
	...
	function upload() {
		...
		if (empty($this->data)) {
			...
			if (!$result)
			{
				$this->data['Team']['logo_url'] = '/img/uploads/logos/' . $this->Upload->result;
