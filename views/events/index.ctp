...
<div class="events index">
	...
	<div class="events index" >
			<?php echo $form->create('Event', array('action' => 'index'));?>
			...
			<table>
				<tr>
					...
					<td>
						...
						<?php echo $form->select('game_id', $games, (!empty($this->data['Event']['game_id']) ? $this->data['Event']['game_id'] : ''), array(), __('All',true)); ?>
					</td>
...
<table cellpadding="5" cellspacing="5" class="list" style="width:650px">
	...
	<tr class="altrow">
					...
					<td style="text-align:center;width:20px">
						<?php echo $html->image($event['Game']['icon']); ?>
					</td>	
					<td>			
						<?php echo $html->link($event['Event']['name'], array('controller'=> 'events', 'action'=>'view', $event['Event']['id'])); ?>
					</td>
					...
					<td rowspan="2" style="text-align:center;width:75px">
						...
						<table cellspacing="0" cellpadding="0">
								<tr>
									...
									<td style="vertical-align: middle;">
										<?php echo $html->link(__("Sign-up", true), array('controller'=> 'events', 'action'=>'sign', $event['Event']['id'])); ?>
									</td>																
								</tr>
						</table>
						<?php endif; ?>
				...
				<tr>
					<td colspan="4">
						<table cellpadding="1" cellspacing="1" class="list">
							<tr>
								<td style="width:25%">
									<?php __('Created'); ?>: <?php echo substr($event['Event']['created'],0,10); ?>
								</td>
								<td style="width:2.5%">
									&nbsp;
								</td>
								<td style="width:25%">
									<?php __('Start'); ?>: <?php echo (empty($event['Event']['startdate']) ? "" : substr($event['Event']['startdate'],5,11)); ?>
								</td>
								<td style="width:2.5%">
									&nbsp;
								</td>
								<td style="width:20%">
									<?php __('End'); ?>: <?php echo (empty($event['Event']['enddate']) ? "" : substr($event['Event']['enddate'],5,5)); ?>
								</td>
								<td style="width:2%">
									&nbsp;
								</td>
								<td style="width:23%">
									<?php __('Administrator'); ?>: <?php echo $event['User']['name']; ?>
								</td>
							</tr>
						</table>
					</td>	
